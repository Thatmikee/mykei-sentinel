...
10. **CookieYes 403 root cause confirmed 2026-08-04, not fixable from this repo.** Corrected from an earlier wrong entry here (which claimed a CSP fix resolved this — impossible, CSP can't cause a third-party 403). Real cause: the CookieYes client ID `3827feeaf5b6081f66aa049a` is not recognised by CookieYes's own system (curl-confirmed via a known-live comparison ID returning 200 vs ours/random IDs all returning identical 403s) — almost certainly an expired/suspended account or deleted site registration on CookieYes's side. No consent banner renders at all right now. Needs Michael to check the CookieYes dashboard directly (billing/account status, re-register the domain if needed) — see `docs/audit/2026-08-03-infrastructure-audit.md` §5 for full evidence. GA4 stays deliberately blocked in CSP until this is fixed and re-verified in a real browser, not assumed.

11. **FIXED 2026-08-10.** `access-control-allow-origin: *` was removed via a `! Access-Control-Allow-Origin` directive in `public/_headers` (commit `7a67fff`) — Cloudflare Pages supports stripping a platform-injected header this way, no dashboard access needed after all. Confirmed gone on live production via curl.
